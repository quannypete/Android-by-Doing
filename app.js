...
const exercises = [
  [1,0,"Hello Android","Create your first Compose screen and center a greeting.",["@Composable","Text","Modifier"],["Create an Empty Activity project using Compose.","Display “Hello Peter!” in the center of the screen.","Use MaterialTheme and no XML layouts."],"Make the text bold, blue, and 30sp.",["composeCourse","composeTutorial","modifiers"]],
...
const challenges = [
  ["Stopwatch","Start, pause, reset, and preserve elapsed time across rotation."],
  ["Tip Calculator","Calculate tip and total, validate input, and support split bills."],
  ["Shopping List","Add, edit, check off, sort, and persist shopping items."],
  ["Baseball Pitch Counter","Track balls, strikes, outs, pitch count, and inning resets."],
  ["Batting Lineup Manager","Reorder players, assign positions, and rotate innings."],
  ["Gamecast Score Bug","Recreate a compact live-score component from memory."],
  ["Android Calculator","Implement keypad behavior, operations, formatting, and state."],
  ["Google Keep Mini","Build notes with colors, search, pinning, and local persistence."],
  ["Settings Clone","Recreate a polished settings screen with working preferences."],
  ["Play Store Home","Build a responsive app catalog with carousels and detail navigation."]
];
...
function renderExercises() {
  ...
  els.grid.innerHTML = filtered.map(ex => `
    <article class="exercise-card ${progress.has(ex.id) ? "completed" : ""}" data-id="${ex.id}">
      ...
      <div class="card-actions"><button class="open-exercise" type="button" data-open="${ex.id}">Open lesson →</button>
...
els.grid.addEventListener("click", e => {
  ...
  if (open) openExercise(Number(open.dataset.open));
});
...
els.challengeGrid.innerHTML = challenges.map(([title,desc])=>`<article class="challenge-card"><h3>${title}</h3><p>${desc}</p></article>`).join("");
